...
/// Summarize one empirical similarity-score distribution.
...
pub fn compare_distributions(
    ...
    let ks_statistic = ks_two_sample_statistic(&previous.scores, &current.scores);
    Ok(DistributionComparison {
        ...
        wasserstein_1d: wasserstein_1d(&previous.scores, &current.scores),
    })
}
...
fn ks_two_sample_statistic(left: &[f64], right: &[f64]) -> f64 {
...
/// Compute the one-dimensional empirical Wasserstein distance.
fn wasserstein_1d(left: &[f64], right: &[f64]) -> f64 {
    if left.is_empty() || right.is_empty() {
        return f64::NAN;
    }
    let mut left = left.to_vec();
...
mod tests {
    ...
    fn ks_statistic_detects_separated_samples() {
        ...
        assert!((ks_two_sample_statistic(&left, &right) - 1.0).abs() < f64::EPSILON);
    }
    ...
    fn wasserstein_matches_shift_for_equal_samples() {
        ...
        assert!((wasserstein_1d(&left, &right) - 1.0).abs() < 1e-12);
    }
}
