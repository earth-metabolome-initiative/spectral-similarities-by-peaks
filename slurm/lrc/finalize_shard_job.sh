#!/bin/bash
#SBATCH --job-name=spectral-finalize-shard
...
#SBATCH --output=/global/scratch/users/%u/spectral-similarities-by-peaks/logs/finalize_shard_%A_%a.out
#SBATCH --error=/global/scratch/users/%u/spectral-similarities-by-peaks/logs/finalize_shard_%A_%a.err
...
CONFIG_INDEX="${SLURM_ARRAY_TASK_ID:?finalize_shard_job.sh must be launched as an sbatch array}"
...
echo "Command:      target/release/spectral-similarities-by-peaks finalize-shard --config-index $CONFIG_INDEX $*"

target/release/spectral-similarities-by-peaks finalize-shard \
